...
def isclassIncode(className, pre):
...
def getInfoFromRecord(record):
    paraTypeList = getParaList(record.methodName)
...
class MethodNode:
    def __init__(self,vmname, methodID, methodName, classID, className, infoCount, paraTypeList):
        self.vmname = vmname
...
class KiekerParser:
    ...
    def firstParse(self, contentList, pre):
        ...
        for eachLine in contentList:
            ...
            if tmp[0] == '$1':
                ...
                if isclassIncode(className, pre):
	...
	#begin: sorting records using sequence ID  for slice
        ...
        for record in self.recordList:
            ...
            if methodName not in self.methodNodeDict:
                ...
                infoCount = getInfoFromRecord(record)
                ...
                oneMethodNode = MethodNode(record.vmname, methodNodeIndex, methodName, self.classNodeDict[className], className, infoCount, paraTypeList)
...
#python pro.py logdir  pre="org.ibatis.jpetstore"  outcommucost.csv
if __name__ == "__main__":
    ...
    myparser.firstParse(totalContentList, pre) #generate recordlist, sessionList.thodDict,methodList,classDict,classList
    ...
    genClassDeps(outFileName, myparser.classNodeList, classEdgeList)
